...
class CustomAIFollowUpGenerator:
    ...
    def generate_questions(
        ...
        try:
            # Get animal type first - CRITICAL for context
            animal_type = patient_info.get('animal_type', patient_info.get('species', 'animal')).lower()
            
            # Filter database matches to ONLY include diseases for this animal
            animal_specific_matches = [
                disease for disease in database_matches
                if animal_type in [s.lower() for s in disease.get('affected_species', [])]
            ]
            
            # Build context from patient info and symptoms with STRONG animal focus
            context_parts = []
            
            # Add patient info with emphasis on animal type
            if patient_info:
                age = patient_info.get('age', 'unknown age')
                # Use the actual animal type (cat, dog, etc.) not generic "animal"
                context_parts.append(f"{animal_type} patient {age} years old")
            ...
            # Add animal-specific disease context
            if animal_specific_matches:
                top_disease = animal_specific_matches[0].get('name', '')
                if top_disease:
                    context_parts.append(f"suspected {animal_type} disease: {top_disease}")
            ...
            generated_questions = self._generate_with_model(
                context_text, 
                max_questions * 2  # Generate more, then filter
            )
            
            # Filter out questions mentioning wrong animals
            filtered_questions = []
            wrong_animals = ['dog', 'cat', 'bird', 'rabbit', 'hamster', 'guinea pig', 
                            'ferret', 'reptile', 'horse', 'cow', 'goat', 'sheep', 'pig']
            if animal_type.lower() in wrong_animals:
                wrong_animals.remove(animal_type.lower())
            
            for question_text in generated_questions:
                question_lower = question_text.lower()
                # Skip if question mentions wrong animal
                if any(wrong_animal in question_lower for wrong_animal in wrong_animals):
                    continue
                # Replace generic "pet" with specific animal
                question_text = question_text.replace('pet', animal_type)
                question_text = question_text.replace('animal', animal_type)
                filtered_questions.append(question_text)
            
            # Convert to AIFollowUpQuestion objects
            ai_questions = []
            for i, question_text in enumerate(filtered_questions[:max_questions]):
                ai_questions.append(AIFollowUpQuestion(
                    ...
                    reasoning=f'AI-generated question for {animal_type} based on symptoms',
    ...
    def _add_critical_questions(self, questions: List[AIFollowUpQuestion], 
        ...
        # Get specific animal type, not generic
        animal = patient_info.get('animal_type', patient_info.get('species', 'pet')).lower()
