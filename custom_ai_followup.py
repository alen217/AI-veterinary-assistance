...
class CustomAIFollowUpGenerator:
    ...
    def generate_questions(
        ...
        try:
            ...
            if patient_info:
                ...
                species = patient_info.get('animal_type', patient_info.get('species', 'animal'))
            ...
            # Add top disease candidates so generation stays diagnosis-focused
            top_disease_names = [d.get("name") for d in (database_matches or [])[:3] if d.get("name")]
            if top_disease_names:
                context_parts.append(f"possible conditions: {', '.join(top_disease_names)}")
            ...
            known_symptoms = self._collect_known_symptoms(symptoms, database_matches)
            ai_questions = []
            for question_text in generated_questions:
                if question_text in previous_answers:
                    continue
                if not self._is_relevant_question(question_text, known_symptoms, animal):
                    continue
                ...
                ai_questions.append(AIFollowUpQuestion(
                    ...
                    category=category,
                    priority=self._calculate_priority(question_text, symptoms, database_matches),
                    reasoning='Generated from current symptoms and top candidate diseases',
                    expected_answer_type=self._infer_expected_answer_type(category)
                ))

            # Post-process: add diagnosis-critical and disease-specific checks.
            ai_questions = self._add_critical_questions(ai_questions, symptoms, patient_info)
            ai_questions = self._add_disease_specific_questions(
                ai_questions,
                symptoms,
                database_matches,
                patient_info,
            ...
            ai_questions = self._deduplicate_questions(ai_questions)
            # Sort by priority
    ...
    @staticmethod
    def _collect_known_symptoms(symptoms: List[Dict], database_matches: List[Dict]) -> Set[str]:
        """Collect symptom phrases from extracted symptoms and top DB matches."""
        known: Set[str] = set()

        for symptom in symptoms:
            if isinstance(symptom, dict):
                value = symptom.get("symptom", "")
            else:
                value = str(symptom)
            if value:
                known.add(value.lower())
                known.add(value.replace("_", " ").lower())

        for disease in (database_matches or [])[:3]:
            for sym in disease.get("common_symptoms", []) or []:
                if sym:
                    known.add(sym.lower())
                    known.add(sym.replace("_", " ").lower())

        return known

    @staticmethod
    def _is_relevant_question(question: str, known_symptoms: Set[str], animal: str) -> bool:
        """
        Keep questions tied to current symptoms or essential triage prompts.
        This avoids off-topic model outputs in the UI.
        """
        q_lower = question.lower()

        if any(sym and sym in q_lower for sym in known_symptoms):
            return True
    ...
    def _infer_category(question: str) -> str:
        q_lower = question.lower()
        if any(key in q_lower for key in ["how long", "how severe", "how often", "getting worse"]):
            return "symptom_details"
        if any(key in q_lower for key in ["medication", "allerg", "surgery", "history"]):
            return "medical_history"
        if any(key in q_lower for key in ["diet", "food", "environment", "routine"]):
            return "lifestyle"
        return "disease_confirmation"

    @staticmethod
    def _infer_expected_answer_type(category: str) -> str:
        if category in {"disease_confirmation", "additional_symptoms"}:
            return "yes/no"
        if category == "symptom_details":
            return "descriptive"
        return "descriptive"

    def _add_disease_specific_questions(
        self,
        ...
        """
        Add high-value confirmation checks from top-ranked diseases when missing.
        """
        animal = patient_info.get("animal_type", "pet")
        present_symptoms = {
            (s.get("symptom", "") if isinstance(s, dict) else str(s)).lower()
            for s in symptoms
        }

        for idx, disease in enumerate((database_matches or [])[:2]):
            disease_name = disease.get("name", "the suspected condition")
            for sym in disease.get("common_symptoms", []) or []:
                if not sym:
                    continue
                if sym.lower() in present_symptoms:
                    continue

                question_text = f"Has your {animal} shown {sym.replace('_', ' ')}?"
                if question_text in previous_answers:
                    continue
                ...
                questions.append(
                    AIFollowUpQuestion(
                        ...
                        priority=5 if idx == 0 else 4,
                        reasoning=f"Confirms or rules out {disease_name}",
                        expected_answer_type="yes/no",
                    )
                )
                # Limit to one new question per disease to avoid flooding.
                break

        return questions

    @staticmethod
    def _deduplicate_questions(questions: List[AIFollowUpQuestion]) -> List[AIFollowUpQuestion]:
        seen = set()
        deduped: List[AIFollowUpQuestion] = []
        for q in questions:
