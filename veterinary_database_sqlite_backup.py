...
class VeterinaryDatabase:
    ...
    def ensure_default_users(self):
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute("INSERT INTO users (username, password, role) VALUES (?, ?, ?)",
                         (admin_username, hashed_admin, "admin"))
        
        cursor.execute("SELECT COUNT(*) FROM users WHERE username = ?", (user_username,))
        if cursor.fetchone()[0] == 0:
            hashed_user = hashlib.sha256(user_password.encode()).hexdigest()
            cursor.execute("INSERT INTO users (username, password, role) VALUES (?, ?, ?)",
                         (user_username, hashed_user, "user"))
    ...
    # Property to mimic MongoDB users collection for compatibility
    @property
    def users(self):
        """Return a simple object that mimics MongoDB users collection"""
        class UsersCollection:
            ...
            def delete_one(self, query):
                if 'username' in query:
                    success = self.db.delete_user(query['username'])
                    class Result:
